...
impl BinanceHandler {
    ...
    fn public_key_to_address(&self, public_key_bytes: &[u8]) -> Result<String> {
        // Binance Smart Chain (BSC) uses identical address generation to Ethereum:
        ...
        let verifying_key = VerifyingKey::from_sec1_bytes(public_key_bytes)
            .context("Invalid public key for Binance address generation")?;
...
impl BlockchainHandler for BinanceHandler {
    fn derive_from_mnemonic(
        ...
        let derivation_path = match custom_path {
            ...
            None => {
                // Binance Smart Chain uses BIP-44 derivation path: m/44'/714'/0'/0/0
                format!("m/44'/714'/{}'/{}/{}", account, 0, address_index)
            },
        };
    ...
    fn derive_from_private_key(&self, private_key: &str) -> Result<WalletKeys> {
        ...
        if private_key_bytes.len() != 32 {
            return Err(anyhow::anyhow!("Binance private key must be 32 bytes"));
        }
...
mod tests {
    ...
    #[test]
    fn test_binance_address_validation() {
        let handler = BinanceHandler::new();

        // Generate a real Binance address to test validation
    ...
    #[test]
    fn test_binance_private_key_derivation() {
        let handler = BinanceHandler::new();
        ...
        assert!(result.is_ok());
        let wallet_keys = result.unwrap();
        println!("Generated Binance address: {}", wallet_keys.address);
    ...
    #[test]
    fn test_binance_uses_correct_derivation_path() {
        let handler = BinanceHandler::new();

        // Test that Binance uses coin type 714
        ...
        // Verify it uses Binance coin type (714)
        assert_eq!(wallet_keys.derivation_path, "m/44'/714'/0'/0/0");
        println!("Binance derivation path: {}", wallet_keys.derivation_path);
    }
}