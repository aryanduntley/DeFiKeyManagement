...
impl PolygonHandler {
    ...
    fn public_key_to_address(&self, public_key_bytes: &[u8]) -> Result<String> {
        ...
        let verifying_key = VerifyingKey::from_sec1_bytes(public_key_bytes)
            .context("Invalid public key for Polygon address generation")?;
...
impl BlockchainHandler for PolygonHandler {
    fn derive_from_mnemonic(
        ...
        let derivation_path = match custom_path {
            ...
            None => SupportedBlockchain::Polygon.get_default_derivation_path(account, address_index),
        };
        ...
        // Generate Polygon address from public key (ETH-compatible)
    ...
    fn derive_from_private_key(&self, private_key: &str) -> Result<WalletKeys> {
        ...
        if private_key_bytes.len() != 32 {
            return Err(anyhow::anyhow!("Polygon private key must be 32 bytes"));
        }
...
mod tests {
    ...
    #[test]
    fn test_polygon_address_validation() {
        let handler = PolygonHandler::new();

        // Generate a real Polygon address to test validation
    ...
    #[test]
    fn test_polygon_private_key_derivation() {
        let handler = PolygonHandler::new();
        ...
        assert!(result.is_ok());
        let wallet_keys = result.unwrap();
        println!("Generated Polygon address: {}", wallet_keys.address);
